...
int MyLogger::MakePrefix(MyLogLevel logLevel) {
  ...
  if (m_PrefixBuf[0] != 0 && m_SuffixBuf[0] != 0) {
    ...
    return sprintf(m_LogBuf, "[%llu][%08x][%s][%s] ", timestamp, (unsigned int)tid, levelStr, m_SuffixBuf);
  } else {
    return sprintf(m_LogBuf, "[%llu][%08x][%s] ", timestamp, (unsigned int)tid, levelStr);
  }
}
